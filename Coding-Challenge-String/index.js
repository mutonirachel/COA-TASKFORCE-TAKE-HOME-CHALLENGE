// 4.3 Coding Challenge 2: String Transformation :
    ...
    if (length % 15 === 0) {                // Divisible by 15: reverse and then replace each character with its ASCII code
    ...
    } else if (length % 5 === 0) {        // Divisible by 5: replace each character with its ASCII code
        return input.split('').map(char => char.charCodeAt(0)).join(' ');
    } else if (length % 3 === 0) {       // Divisible by 3: reverse the string
